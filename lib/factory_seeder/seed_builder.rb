...
module FactorySeeder
  class SeedBuilder
    ...
    def parameter(name, type: :string, required: false, default: nil, allowed_values: nil, min: nil, max: nil, description: nil)
    ...
    def validate_parameter_options(type, allowed_values:, min:, max:)
      ...
      when :boolean
        raise ArgumentError, "min, max, and allowed_values are not valid for boolean type" if min || max || allowed_values
      when :array
