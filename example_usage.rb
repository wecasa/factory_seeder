...
puts "\n1. Définition d'un seed simple:"
FactorySeeder.define_seed(:hello_world, ->(builder) {
  builder
...
FactorySeeder.define_seed(:create_sample_data, ->(builder) {
  builder
    ...
    .symbol_param(:user_type, required: false, default: :regular, allowed_values: [:regular, :premium, :admin], description: "Type d'utilisateur")
    .array_param(:tags, required: false, default: ['ruby', 'rails'], description: "Tags à appliquer")
}) do |user_count:, create_posts: true, user_type: :regular, tags: ['ruby', 'rails']|
  ...
  user_count.times do |i|
    ...
    if create_posts
      puts "    📄 Créé post ##{i + 1} avec tags: #{tags.join(', ')}"
    end
  end
